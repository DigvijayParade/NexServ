...
adminRouter.post('/workers/:id/approve', authenticateUser, requireRole(['admin']), async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  try {
...
adminRouter.get('/dashboard/stats', authenticateUser, requireRole(['admin']), async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  try {
